...
data = pd.read_csv("Rand_Data.csv",skiprows = None , header = None )
data1 = pd.read_csv("Rand_Data.csv",skiprows = None , header = None )
...
for i in range(200,len(data)):
    x_train.append(data[i-200:i,0].reshape(-1,1))
    y_train.append(data[i,1])
#    x_train,y_train = np.array(x_train), np.array(y_train)
x_train , y_train = np.array(x_train) , np.array(y_train)

data1 = data1.values
for i in range(200,len(data1)):
    x_train1.append(data1[i-200:i,0].reshape(-1,1))
...
model.add(LSTM(units =50,return_sequences = True, input_shape = (x_train.shape[1], x_train.shape[2]),activation='sigmoid'))    
